...
bar4<-ggplot(total, aes(x=decilmanagement, y=opprofit_millions))+ 
  labs(x="Decile of Management", y="Av. Operating Profit") +stat_summary(fun = "mean", geom = "bar")
...
total$RDi_m<-total$II1R10C2/1000
total$RDi_m_per_empl=(total$RDi_m/total$empltot)
total$RDi_m_per_empl1=(total$RDi_m_per_empl)+1
total$lnRDi_m_per_empl1=log(total$RDi_m_per_empl1) ####This the variable for RD
...
totalog2<-total[!(total$empltot=="0"|total$IV1R11C2=="0"| total$valueadded=="0"),]#|total$II1R10C2=="0"


###########  Plant Management Scores and Performance (Bloom, 2019: Table 1)

#Log(Production/employees)

m1 <- plm(lnprod ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,
                      index = "ciiu4",
                      effect = "individual", 
                      model = "within",
                      data = totalog1)

m2 <- plm(lnprod ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,
                      index = "dpto",
                      effect = "individual", 
                      model = "within",
                      data = totalog1)

m3<-lm(lnprod ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,data=totalog1)

#Log(sales/employees)

m4 <- plm( lnsales~ Management+lnfape+lniipe+empltot+staff_with_college_degree,
                      index = "ciiu4",
                      effect = "individual", 
                      model = "within",
                      data = totalog1)

m5 <- plm(lnsales ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,
                      index = "dpto",
                      effect = "individual", 
                      model = "within",
                      data = totalog1)

m6<-lm(lnsales ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,data=totalog1)

#Log(sales/employees)

m7 <- plm(profoversales~ Management+lnfape+lniipe+empltot+staff_with_college_degree,
                      index = "ciiu4",
                      effect = "individual", 
                      model = "within",
                      data = totalog1)

m8 <- plm(profoversales ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,
                      index = "dpto",
                      effect = "individual", 
                      model = "within",
                      data = totalog1)

m9<-lm(profoversales ~ Management+lnfape+lniipe+empltot+staff_with_college_degree,data=totalog1)
...
# LISTING VARIABLES FOR FIXED EFFECTS
...
totalog1<-total[!(total$empltot=="0" | total$fixedassets=="0" |total$IV1R11C2=="0"| total$valprod=="0"),]
totalog2<-total[!(total$empltot=="0"|total$IV1R11C2=="0"| total$valueadded=="0"|total$II1R4C2=="0"),]

#Omitting special cases: NA, inf
totalog3<-totalog1[!(total$valueadded=="0"|total$sales=="0"),]
totalog3<-subset(totalog3,select=c(NORDEMP,log_Value_Added_per_Employee,Management,ciiu4,ciiu4_2_d,dpto,lnfape,lnempltot,staff_with_college_degree,profoversales,lerner))
totalog4<-subset(totalog2,select=c(NORDEMP,log_Value_Added_per_Employee,Management,ciiu4,ciiu4_2_d,dpto,lnfape,lnempltot,staff_with_college_degree,profoversales,empltot,lnRDi_m_per_empl1, ICT_per_Employee))
...
m14 <- plm(log_Value_Added_per_Employee ~ Management + lnfape+lnempltot+staff_with_college_degree, 
                      ...
                      data = totalog3)
#FE:No
m15<-lm(log_Value_Added_per_Employee ~ Management + lnfape+lnempltot+staff_with_college_degree,data =totalog3)
...
m17 <- plm(profoversales ~ Management + lnfape+lnempltot+staff_with_college_degree, 
                      ...
                      data = totalog3)
#FE:No
m18<-lm(profoversales ~ Management + lnfape+lnempltot+staff_with_college_degree,data =totalog3)


#FE:yes, with ciiu4. Dependent Variable:RDi
m19 <- plm(lnRDi_m_per_empl1~ Management + lnfape+lnempltot+staff_with_college_degree, 
                      index = c("ciiu4"),
                      effect = "individual", 
                      model = "within",
                      data = totalog4)
...
#FE:yes, with ciiu4. Dependent Variable:RDi
m20 <- plm(lnRDi_m_per_empl1~ Management + lnfape+lnempltot+staff_with_college_degree, 
                      index = c("dpto"),
                      effect = "individual", 
                      model = "within",
                      data = totalog4)
...
#FE:No
m21<-lm(lnRDi_m_per_empl1~ Management + lnfape+lnempltot+staff_with_college_degree,data =totalog4)
...
texreg(list(m13, m14,m15,m16,m17,m18,m19,m20,m21),caption.above = TRUE, caption="Firm Management Scores and Performance",fontsize = "small",   custom.header = list("Log(VA/Emp)"=1:3,"(Profits/sales)"=4:6, "RDi/Emp"=7:9),digits =2,omit.coef ="Intercept",custom.coef.names=c("Management","Ln(Cap/Emp)", "Ln(Input/Emp)", "Ln(Employment)","Degree"), custom.model.names =c("1","2","3","4","5","6","7","8","9"),custom.note = ("\\parbox{0.95\\linewidth}{\\vspace{3pt}%stars. \\\\OLS coefficients with standard errors in parentheses. The management score is the unweighted average of the score for each of the 16 questions, where each question is first normalized to be on a 0–1 scale. The sample is all EDIT observations with at least 11 non-missing responses to management questions and a successful match to EAM, which have positive value added, positive employment, and positive imputed capital. The columns 1-3 mean the models with Industry Fixed Effects, Location Fixed Effects and no Fixed Effects, respectively.This also applies for columns 4-6 and 7-9.}")) 
...
totalog2<-total[!(total$empltot=="0"|total$IV1R11C2=="0"| total$valueadded=="0"|total$II1R4C2=="0"),]#|total$II1R10C2=="0"

reg_dpvlog1<-lm(log_Value_Added_per_Employee ~ Management, data =totalog2,)
reg_dpvlog2<-lm(log_Value_Added_per_Employee ~ Management+lnRDi_m_per_empl1, data =totalog2,)
reg_dpvlog3<-lm(log_Value_Added_per_Employee ~ Management+lnRDi_m_per_empl1+ICT_per_Employee, data =totalog2,)
reg_dpvlog4<-lm(log_Value_Added_per_Employee ~ Management+lnRDi_m_per_empl1+ICT_per_Employee+staff_with_college_degree, data =totalog2,)
...
texreg(list(reg_dpvlog1, reg_dpvlog2,reg_dpvlog3,reg_dpvlog4),caption.above = TRUE, caption="Drivers of Productivity Variation",fontsize = "normalsize",   custom.header = list("Log(VA/Emp)"=1:4),digits =3,omit.coef ="Intercept",custom.coef.names=c("Management","RDi", "ICT/Emp","Degree"), custom.model.names =c("1","2","3","4"),custom.note = ("\\parbox{0.6\\linewidth}{\\vspace{3pt}%stars. \\\\OLS coefficients with standard errors in parentheses.Dependent variable is firm level log(Value Added/Employment). Right-hand-side variables are Management score, RDi is measured as log(1+RDi intensity), where RDi intensity is the total domestic Research, Development and innovation expenditure in 2018 divided by total domestic employment, ICT/Emp is investment per worker (1,000 × spending on information and communication technology hardware and software per employee), Degree is measured by the share of employees (managers and non-managers) with a college degree.}"))
...
m22<-lm( Management ~ lerner,data =totalog3)
