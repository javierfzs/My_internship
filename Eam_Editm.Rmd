...
##JAVIER FERNANDEZ SALAMANCA estpa alias Don Javi AND LEONARDO IACOVONE###
...
Eam_2018_2<-aggregate(list(Eam_2018_1$CONSIN2, Eam_2018_1$ACTIVFI,Eam_2018_1$PERTOTAL,Eam_2018_1$PRODBR2,Eam_2018_1$c7c4r14,Eam_2018_1$VALAGRI), by=list(Eam_2018_1$nordemp),FUN=sum,na.rm=TRUE) #, na.rm=FALSE
...
#listing variables to plot (Productivity,Exports,Profit, R&D and i) and some? which?

total$logempltot2<-log(total$empltot2)
total$valprod<-log(total$valprod/total$empltot)
...
#List of variables for Manegement score and performance
#With Logarithms 
#1. staff_with_college_degree: percent of staff with college degree
...
#4. Total Score (1-16): total$Management
...
#3.Log (Investment on ITC) - 2018:total$II1R4C2
total$logICT<-log(total$II1R4C2)
...
reg_msplog <-lm(lnprod ~ lnfape+lniipe+empltot+Management+staff_with_college_degree,data=totalog1)
#summary(reg_msplog)


stargazer(reg_msplog,
          header = FALSE,
          title = "Management Score and Performance",
          dep.var.labels = "Log(Production/employees)",
...
totalog2<-total[!(total$IV1R11C2=="0"| total$valueadded=="0"|total$II1R10C2=="0"|total$II1R4C2=="0"),]


reg_dpvlog<-lm(log_Value_Added_per_Employee ~ Management+log_RD_innovation+logICT+staff_with_college_degree, data =totalog2)
