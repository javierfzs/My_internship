...
bar3<-ggplot(total, aes(x=decilmanagement, y=rdi_millions))+ 
  labs(x="Decile of Management", y=" Av. R&D+i") +stat_summary(fun = "mean", geom = "bar")
...
total$lnRDi_m_per_empl1=log(total$RDi_m_per_empl1) ####This the variable for R&D
...
totalog4<-subset(totalog2,select=c(NORDEMP,log_Value_Added_per_Employee,Management,ciiu4,ciiu4_2_d,dpto,lnfape,lnempltot,staff_with_college_degree,profoversales,empltot,lnRDi_m_per_empl1))
...
reg_dpvlog3<-lm(log_Value_Added_per_Employee ~ Management+lnRDi_m_per_empl1+logICT_per_Employee, data =totalog2,)
reg_dpvlog4<-lm(log_Value_Added_per_Employee ~ Management+lnRDi_m_per_empl1+logICT_per_Employee+staff_with_college_degree, data =totalog2,)
